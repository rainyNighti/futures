...
# --- 3. 粗筛选阶段 ---
def run_coarse_filtering(X_train, y_train, mi_keep_ratio=0.3, corr_threshold=0.9):
    """执行粗筛选流程"""
    # 移除高度相关特征
    mi_scores = pd.Series(
    ...
    for col1, col2, p_score in high_corr_pairs:
        ...
        if mi_col1 < mi_col2:
            to_drop.add(col1)
            print(f"高相关对: ('{col1}', '{col2}'). 决策: 移除 '{col1}' (MI: {mi_col1:.4f} < {mi_col2:.4f}), (P-score: {p_score})")
        else:
            to_drop.add(col2)
            print(f"高相关对: ('{col1}', '{col2}'). 决策: 移除 '{col2}' (MI: {mi_col2:.4f} <= {mi_col1:.4f}), (P-score: {p_score})")

    selected_features_corr = X_train.columns.drop(list(to_drop))
    print(f"\n移除高相关特征后，剩余特征: {len(selected_features_corr)}")
    ...
    print(f"互信息筛选后，最终剩余特征: {len(final_selected_features)}")
    print(f"最终选择的特征: {final_selected_features.to_list()}")
...
def run_null_importance(X, y, features, n_runs=100):
    ...
    print(f"Null Importance筛选后剩余特征: {len(significant_features)}")
    print(f"Null Importance筛选后剩余特征: {significant_features}")
    print("--- 高级降噪完成 ---")
...
def find_optimal_features_rfe(X, y, features, n_to_test):
    ...
    for n in tqdm(n_to_test, desc="Testing N features with RFE"):
        ...
        print(f"RFE with n={n}: Average pps = {avg_score:.4f}")

    results_df = pd.DataFrame(results)
    best_n = results_df.loc[results_df['avg_pps'].idxmin()]
    
    print(f"\n最优特征数量为: {int(best_n['n_features'])} (pps: {best_n['avg_pps']:.4f})")
...
if __name__ == '__main__':
    ...
    # --- 1. 数据准备 ---
    target_column = 'T_5'
    df = pd.read_pickle('cache_feature_engineering/brent.pkl')
    y = df[target_column]
    ...
    print(f"训练集大小: {X_train_main.shape}, 测试集大小: {X_test_main.shape}")
    # 阶段一：粗筛选
    if True:
        X_train_coarse, X_test_coarse, coarse_features = run_coarse_filtering(
            X_train_main, y_train_main, X_test_main
        )
    else:
        X_train_coarse = X_train_main
        X_test_coarse = X_test_main
        coarse_features = list(X_train_coarse.columns)
    ...
    print(f"使用RFE选择的 {len(rfe_selected_features)} 个特征，在最终测试集上的pps: {pps_rfe:.4f}")